...
class CBtn {
    CBtn() {}
  public:
    CBtn(byte nPin, byte nFrom, byte nTo) {
      m_bPressed = false;
      m_lastTime = 0;
      m_nPin = nPin;
      m_nFrom = nFrom;
    ...
    bool Pressed() {
      const byte nVal = (byte)(analogRead(m_nPin) / 4);
      bool bPressed = (nVal >= m_nFrom && nVal < m_nTo), ret = false;
  ...
  private:
    ...
    byte m_nPin, m_nFrom, m_nTo;
};

CBtn btnMode1(PIN_DETUNE_SWITCH, 120, 140);
CBtn btnMode2(PIN_DETUNE_SWITCH, 170, 190);
...
static const byte OUT_SHIFT_DATA_PIN = 5;  // -> pin 14 of 74HC595 - data input, slave in SI (DS)
...
byte ay_data[2], ay_ctrl = 0;

#define BIT_AY_RESET 0
#define BIT_AY0_BC1 1
#define BIT_AY0_BDIR 2
#define BIT_AY1_BC1 3
#define BIT_AY1_BDIR 4

byte volumeA[2], volumeB[2], volumeC[2];
...
void resetAY() {
  ...
  bitClear(ay_ctrl, BIT_AY0_BC1); // reset BC1
  bitClear(ay_ctrl, BIT_AY0_BDIR); // reset BDIR
  bitClear(ay_ctrl, BIT_AY1_BC1); // reset BC1
  bitClear(ay_ctrl, BIT_AY1_BDIR); // reset BDIR
  bitClear(ay_ctrl, BIT_AY_RESET); // reset AY
  sendAY();
  delay(100);
  bitSet(ay_ctrl, BIT_AY_RESET); // unreset AY
  sendAY();
  delay(100);
...
void writeReg_AY(byte ay, byte port, byte data) { // ay = 0, 1
  byte bit_bc1, bit_bdir;

  switch (ay) {
    case 0:
      bit_bc1 = BIT_AY0_BC1;
      bit_bdir = BIT_AY0_BDIR;
      break;
    case 1:
      bit_bc1 = BIT_AY1_BC1;
      bit_bdir = BIT_AY1_BDIR;
      break;
    default:
      return;
  }
  ...
  if (port == 8 || port == 9 || port == 10) {
    if (port == 8) volumeA[ay] = data;
    if (port == 9) volumeB[ay] = data;
    if (port == 10) volumeC[ay] = data;
  }

  bitClear(ay_ctrl, bit_bc1); // set zero to BC1, BDIR
  bitClear(ay_ctrl, bit_bdir);
  sendAY();

  ay_data[ay] = (port & B00001111); // set AY port 0...15 to D0...D3, set zero to D4...D7
  sendAY();

  bitSet(ay_ctrl, bit_bc1); // set 1 to BC1, BDIR
  bitSet(ay_ctrl, bit_bdir);
  sendAY();
  delayMicroseconds(5);
  ...
  ay_data[ay] = data; // set data bits to D0...D7
  sendAY();
  delayMicroseconds(5);

  bitSet(ay_ctrl, bit_bdir); // set 1 to BDIR
  sendAY();
  delayMicroseconds(5);
...
void sendAY() {
  out_595_3byte(ay_data[0], ay_data[1], ay_ctrl);
}

void out_595_3byte(byte b1, byte b2, byte hi) {
  digitalWrite(OUT_SHIFT_LATCH_PIN, LOW);
  out_595_byte(hi);
  out_595_byte(b2);
  out_595_byte(b1);
  digitalWrite(OUT_SHIFT_LATCH_PIN, HIGH);
...
void out_595_byte(byte x) {
  ...
  for (int i = 0; i < 8; i++) {
    if (ii & x)
      digitalWrite(OUT_SHIFT_DATA_PIN, HIGH);
    else
      digitalWrite(OUT_SHIFT_DATA_PIN, LOW);
    ii >>= 1;
    digitalWrite(OUT_SHIFT_CLOCK_PIN, HIGH);
    digitalWrite(OUT_SHIFT_CLOCK_PIN, LOW);
  }
}

// Main code ----------------------------------------------

static unsigned long lastUpdate = 0, lastDisp = 0;
...
void setup() {
  ...
  pinMode(PIN_DETUNE_SWITCH, INPUT);
  ...
  // Reset AY
  ay_data[0] = ay_data[1] = ay_ctrl = 0;
  resetAY();

  delay(10);

  lastUpdate = lastDisp = millis();
...
void loop() {
  ...
  bool bPressed1 = false, bPressed2 = false;
  unsigned long now = millis();
  const bool bUpdate = (now - lastUpdate) > 10, bDisp = (now - lastDisp) > 40;
  if (bUpdate) {
    update100Hz();
    lastUpdate += 10;
    bPressed1 = btnMode1.Pressed();
    bPressed2 = btnMode2.Pressed();
  }
  if (bPressed1 || bPressed2) {
    ...
    if (bPressed1) {
      ...
      if (g_detuneType == eNoDetune)
        g_detuneType = eDetuneTotal - 1;
      else
        g_detuneType--;
    }
